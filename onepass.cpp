        ...
        if((*iter).label==label)
        {
            // Label already exists, it may or may not have a value
            {
                ...
                else
                {
                    ...
                    cout<<"ERROR - line "<<dec<<linenumber;
                    return 0;
                }
            }
        }
    ...
    while(getline(f,line))
    {
        ...
        if(label[0]!='.')
        {
            ...
            else if(opcode=="RESW")
            {
                ...
                // when lengthCurrentEntry = -1, it tells it to break textRecord, this is a bit hackish but couldn't find a better alternative
                lengthCurrentEntry = -1;
            }
            else if(opcode=="BYTE")
            {
